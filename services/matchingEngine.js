 ...
 * @version 3.2.0 - Fixed multi-JD integration and error handling
 */
...
const JOB_DOMAINS = {
  'network_engineer': {
    primaryKeywords: ['network', 'cisco', 'routing', 'switching', 'infrastructure'],
    skills: {
      protocols: {
        weight: 35,
        skills: ['tcp/ip', 'ospf', 'hsrp', 'vlan', 'dhcp', 'stp', 'bgp', 'snmp']
      },
      tools: {
        weight: 30,
        skills: ['cisco meraki', 'thousandeyes', 'ping', 'traceroute', 'tracert', 'network monitoring']
      },
      technologies: {
        weight: 25,
        skills: ['routing', 'switching', 'lan', 'wan', 'vpn', 'firewalls', 'load balancers']
      },
      certifications: {
        weight: 10,
        skills: ['ccna', 'ccnp', 'network security', 'itil', 'cloud networking']
      }
    }
  },
  'full_stack_developer': {
    primaryKeywords: ['full stack', 'developer', 'web development', 'frontend', 'backend'],
    skills: {
      frontend: {
        weight: 25,
        skills: ['javascript', 'js', 'html5', 'css3', 'react', 'angular', 'vue.js', 'bootstrap']
      },
      backend: {
        weight: 25,
        skills: ['node.js', 'express.js', 'python', 'java', 'django', 'spring', 'restful apis']
      },
      database: {
        weight: 20,
        skills: ['mongodb', 'mysql', 'postgresql', 'nosql', 'sql', 'database design']
      },
      tools: {
        weight: 15,
        skills: ['git', 'github', 'docker', 'aws', 'postman', 'mvc architecture']
      },
      concepts: {
        weight: 15,
        skills: ['rest apis', 'microservices', 'agile', 'version control', 'testing frameworks']
      }
    }
  },
  'software_developer': {
    primaryKeywords: ['software', 'developer', 'programming', 'coding', 'intern'],
    skills: {
      programming: {
        weight: 40,
        skills: ['javascript', 'python', 'java', 'node.js', 'express.js', 'programming fundamentals']
      },
      web_tech: {
        weight: 25,
      ...
      tools: {
        weight: 10,
        skills: ['git', 'version control', 'jwt authentication', 'agile methodologies']
      },
      concepts: {
        weight: 5,
  ...
  'finance_intern': {
    ...
    skills: {
      finance: {
        weight: 40,
        skills: ['financial analysis', 'accounting', 'budgeting', 'financial modeling', 'audit']
      },
      software: {
        weight: 30,
      ...
      domain: {
        weight: 20,
        skills: ['gst', 'taxation', 'financial reporting', 'accounts payable', 'accounts receivable']
      },
      skills: {
        weight: 10,
        skills: ['analytical skills', 'attention to detail', 'communication', 'research']
      }
    }
  }
};
...
const EDUCATION_RELEVANCE = {
  'network_engineer': {
    highly_relevant: ['computer science', 'information technology', 'engineering', 'b.tech', 'm.tech', 'mca', 'bca'],
    somewhat_relevant: ['electronics', 'telecommunications'],
    not_relevant: ['commerce', 'finance', 'accounting', 'mba', 'bba']
  },
  'full_stack_developer': {
    highly_relevant: ['computer science', 'software engineering', 'mca', 'bca', 'b.tech', 'm.tech'],
  ...
  'finance_intern': {
    highly_relevant: ['finance', 'accounting', 'commerce', 'mba', 'bba', 'b.com', 'm.com'],
...
function calculateEducationRelevance(resumeEducation, jobCategory) {
  ...
  if (relevance.highly_relevant.some(edu => education.includes(edu))) {
    return { score: 20, reason: `Highly relevant education: ${resumeEducation}` };
  } else if (relevance.somewhat_relevant.some(edu => education.includes(edu))) {
    return { score: 12, reason: `Somewhat relevant education: ${resumeEducation}` };
  } else if (relevance.not_relevant.some(edu => education.includes(edu))) {
    return { score: 2, reason: `Education mismatch: ${resumeEducation} not suitable for ${jobCategory.replace('_', ' ')}` };
  }
...
function calculateNetworkEngineerScore(resume, job, jobTitle) {
  ...
  Object.entries(networkSkills).forEach(([category, config]) => {
    const categoryMatches = config.skills.filter(skill => {
      return resumeSkills.some(resumeSkill => {
        ...
        if (skill === 'cisco meraki' && (resumeText.includes('cisco meraki') || resumeText.includes('meraki'))) return true;
        if (skill === 'thousandeyes' && (resumeText.includes('thousandeyes') || resumeText.includes('thousand eyes'))) return true;
        if (skill === 'tcp/ip' && (resumeText.includes('tcp/ip') || resumeText.includes('tcpip'))) return true;
        return false;
    ...
    if (categoryMatches.length > 0) {
      reasons.push(`${category}: ${categoryMatches.join(', ')} (${categoryMatches.length}/${config.skills.length})`);
    }
  });

  // Experience bonus for current networking role
  if (resumeText.includes('network engineer') || (resumeText.includes('network') && resumeText.includes('engineer'))) {
    skillScore += 20;
  ...
  const nonTechSkills = ['tally', 'accounting', 'finance', 'excel', 'office', 'bookkeeping', 'taxation', 'digital marketing', 'marketing'];
  ...
  // Final calculation
  const totalScore = Math.min(100, skillScore + experienceScore + educationResult.score - domainPenalty);
  const finalScore = Math.round(Math.max(5, totalScore));

  console.log(`📊 ${resume.candidateName} Network Engineer score: ${finalScore}%`);
...
function calculateFinanceInternScore(resume, job, jobTitle) {
  ...
  if (resumeText.includes('finance intern') || resumeText.includes('biocon') || resumeText.includes('financial')) {
    skillScore += 25;
  ...
  const techSkills = ['javascript', 'python', 'java', 'react', 'node.js', 'programming', 'developer', 'software', 'coding'];
  ...
  const totalScore = Math.min(100, skillScore + experienceScore + educationResult.score - techPenalty);
  const finalScore = Math.round(Math.max(5, totalScore));

  console.log(`📊 ${resume.candidateName} Finance Intern score: ${finalScore}%`);
    ...
    skillsMatch: {
      matched: resumeSkills.filter(skill =>
        Object.values(financeSkills).some(category =>
          category.skills.some(reqSkill => skill.includes(reqSkill.replace(/[^a-z]/g, '')))
        )
      ),
      missing: [],
...
function calculateFullStackScore(resume, job, jobTitle) {
  ...
  // ✅ FIX: Enhanced domain mismatch penalty
  const nonTechSkills = ['tally', 'accounting', 'finance', 'excel', 'office', 'bookkeeping', 'taxation', 'digital marketing'];
  ...
  const totalScore = skillScore + experienceScore + educationScore - domainPenalty;
 ...
 */
async function scoreCandidate(resume, {
  requiredSkills = [],
  minExp = 0,
  maxExp = 100,
  educationLevel = '',
  ...
  const jobData = { requiredSkills, minExp, maxExp, educationLevel, description: jobDescription };
  ...
  try {
    switch (jobCategory) {
      ...
      case 'software_developer':
        ...
        break;
      default:
        result = await calculateEnhancedCandidateScore(resume, jobData, jobTitle);
    }
...
async function scoreMultipleJobs(resume, jobDescriptions) {
  ...
  const results = {};
  console.log(`🎯 Multi-JD scoring for ${resume.candidateName} against ${jobDescriptions.length} jobs`);
    ...
    try {
      ...
      switch (jobCategory) {
        ...
        case 'full_stack_developer':
          ...
          break;
        case 'software_developer':
          result = calculateSoftwareDeveloperScore(resume, jobData, jd.jobTitle);
          break;
        default:
          result = await calculateEnhancedCandidateScore(resume, jobData, jd.jobTitle);
      }
      ...
      console.error(`❌ Failed to score ${resume.candidateName} for ${jd.jobTitle}:`, jobError.message);
...
module.exports = {
  ...
  strictSkillMatch: (resumeSkills, requiredSkills) => ({ matched: [], missing: [] }),
  calculateSkillMatch: () => 0,
  calculateMatchScore: calculateEnhancedCandidateScore,
  _utils: {
    validateSkillsRelevance: () => ({ isRelevant: true, penalty: 0 }),
    validateEducation: () => ({ suitable: true, penalty: 0 })
  }
};
